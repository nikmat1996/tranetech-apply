...
const BlackMonitor = () => {
  ...
  const montitorRef = useRef()
  useEffect(() => {
    const cb = ([{ isIntersecting }]) => {
      if (isIntersecting) {
        montitorRef.current.style.opacity = 1
      }
    }
    const monitorObserver = new IntersectionObserver(cb)
    monitorObserver.observe(montitorRef.current)
  }, [])
  ...
  return (
      <section ref={montitorRef} className='blackMonitor' style={blackMonitor_style}>
